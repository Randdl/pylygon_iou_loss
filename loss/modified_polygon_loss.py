...
def do_something(poly1, poly2):
    ...
    print("get intersects in valid range: ", end - start)
    ...
    Nx = Nx[keep[:, 0], keep[:, 1]]
    Ny = Ny[keep[:, 0], keep[:, 1]]
    # intersections is an N * 2 matrix that contains all paris of valid intersections
    intersections = torch.stack([Nx, Ny], dim=1)
    # print("intersections", intersections.shape, intersections)
    ...
    print("maintain adjacency list: ", end - start)
    ...
    for i in range(keep.shape[0]):
        # for each intersection, remove the 2 original connections for the 2 lines
        # and add the 4 new connections that take into account the intersection point
        xi1 = keep[i, 0]
        xi2 = (xi1 - 1) % p1
        xi3 = keep[i, 1]
        xi4 = (xi3 - 1) % p2
        ...
        for j in range(keep.shape[0]):
            ...
            # condition on pairs of intersections on same line segment
            if i != j and (keep[j, 0] == keep[i, 0] or keep[i, 1] == keep[j, 1]):
                ...
                if keep[j, 0] == keep[i, 0]:
                    ...
                    # why is it xi1? Should it be the actual coordinate of xi1?
                    print("union", union[p1 + p2 + i, 0])
                    print(xi1)
                    if torch.abs(union[p1 + p2 + i, 0] - union[xi1, 0]) < torch.abs(
                            union[p1 + p2 + j, 0] - union[xi1, 0]):  # i is closer
                        con = 1
                    else:
                        con = 0
                    adj[xi1, p1 + p2 + i] = con
                    ...
                    # if the x coordinate of intersection i is closer to xi1 than intersection j, adjust connections
                    if torch.abs(union[p1 + p2 + i, 0] - union[xi3, 0]) < torch.abs(
                            union[p1 + p2 + j, 0] - union[xi3, 0]):  # i is closer
                        con = 1
                    else:
                        con = 0
                    adj[xi3, p1 + p2 + i] = con
            ...
            print("one inner loop: ", end - start)

    endloop = time.time()
    print("big loop: ", endloop - startloop)
    ...
    print("remove points with one collection: ", end - start)
    ...
    print("calculate area: ", end - start)
